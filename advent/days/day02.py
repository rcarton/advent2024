...
def is_safe(report: list[int], skips_left: int = 1) -> bool:
    ...
    for ea, eb in it.pairwise(enumerate(report)):
        ...
        def fallback():
            ...
            if ia == 0:
                return is_safe(report[1:], skips_left - 1) or is_safe(
                    report[:1] + report[1 + 1 :], skips_left - 1
                )
            elif ia == 1:
                return (
                    is_safe(report[1:], skips_left - 1)
                    or is_safe(report_a, skips_left - 1)
                    or is_safe(report_b, skips_left - 1)
                )
            else:
